...
from diffusers.training_utils import EMAModel, _set_state_dict_into_text_encoder, cast_training_params
...
from dataset import InstructPix2PixDataset
...
def log_validation(
    ...
    # run inference
    idx_list = random.sample(list(range(len(val_dataset))), k=args.num_validation_images)
    original_images = []
    edited_images = []
    prompts = []
    for i in idx_list:
        data = val_dataset[i]
        original_images.append(data['original_pixel_values'])
        prompts.append(data['input_ids'])
    ...
    with autocast_ctx:
        for original_image, prompt in zip(original_images, prompts):
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "wandb":
            wandb_table = wandb.Table(columns=["original_image", "edited_image", "edit_prompt"])
            for original_image, prompt, edited_image in zip(original_images, prompts, edited_images):
...
# create custom saving & loading hooks so that `accelerator.save_state(...)` serializes in a nice format
    ...
    # create dataset
    train_dataset, val_dataset, train_dataloader, _ = get_dataloader(args, InstructPix2PixDataset, tokenizer)
    ...
    iclip_noise_scheduler = copy.deepcopy(noise_scheduler)
    iclip_noise_scheduler.num_inference_steps = 20
    zero_timesteps = torch.tensor([0]).long().to(accelerator.device)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                instruct = batch["input_ids"].to(accelerator.device)
                ...
                encoder_hidden_states = text_encoder(instruct)[0]
                ...
                if args.conditioning_dropout_prob is not None:
                    ...
                    if prompt_mask.sum().item() > 0:
                        no_change = torch.stack(
                            random.choices(train_dataset.maintain_list, k=prompt_mask.sum().item()), dim=0
                        ).to(accelerator.device)
                        instruct[prompt_mask[:,0,0]] = no_change
                ...
                # get instructclip image feature, calculate instructclip loss
                curr_timestep = timesteps[0].item()
                prev_timestep = curr_timestep - noise_scheduler.config.num_train_timesteps // iclip_noise_scheduler.num_inference_steps
                latent_out = iclip_noise_scheduler.step(model_pred, curr_timestep, noisy_latents, return_dict=False)[0].to(weight_dtype)
                
                text_feat = iclip.get_text_features(instruct)
                img_feat = iclip.get_image_features(
                    inp=original_image_embeds, 
                    out=latent_out, 
                    inp_t=zero_timesteps.expand(bsz), 
                    out_t=torch.tensor([prev_timestep]).expand(bsz).long().to(accelerator.device)
                )
